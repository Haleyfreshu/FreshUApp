...
export function AdminView({ initialMeals, athletes, orders }) {
  ...
  const deleteMeal = async (id) => {
    ...
    if (!error) setMeals(m => m.filter(x => x.id !== id));
  };
  ...
  const exportCSV = () => {
    const rows = [["Order ID", "Athlete", "Week Of", "Meal", "Calories", "Protein", "Carbs", "Fat", "Price"]];
    orders.forEach(o => o.items.forEach(it => {
      rows.push([o.id, o.athlete_name || "Athlete", o.week_of, it.name, it.calories, it.protein, it.carbs, it.fat, it.price]);
    }));
