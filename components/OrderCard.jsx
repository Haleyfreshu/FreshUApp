  ...
  return (
    <div style={{ background: "var(--fu-card)", borderRadius: 20, padding: 16, marginBottom: 12, boxShadow: "0 2px 14px rgba(0,0,0,0.35)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontWeight: 800, fontSize: 14, color: "var(--fu-text)" }}>{MENUS[order.delivery_day]?.label || "Order"}</div>
          <div style={{ fontSize: 11.5, color: "var(--fu-text-muted)", marginTop: 2 }}>Week of {order.week_of} · {order.items.length} meals</div>
        </div>
        <span style={{
